...
class kmem_cache(hs.heap_structure):
    ...
    def init(self):
        ...
        self.flags = int(self.value["flags"]) & sb.Slub.UNSIGNED_LONG
        ...
        self.object_size = int(
            self.value["object_size"]
        )  # The size of an object without metadata

        self.kmem_cache_cpu_list = (
            []
        )  # list of kmem_cache_cpu objects for that kmem_cache
        # browse the list of gdb.Value (representing the kmem_cache_cpu structure linked list for that kmem_cache)
        for cpu_id, cache_cpu_value in enumerate(
            self.sb.get_all_slab_cache_cpus(self.value)
        ):
            kmem_cache_cpu = kcc.kmem_cache_cpu(self.sb, cpu_id, self, cache_cpu_value)
            self.kmem_cache_cpu_list.append(kmem_cache_cpu)

        self.kmem_cache_node_list = (
            []
        )  # list of kmem_cache_node objects for that kmem_cache
        for node_id in range(self.sb.node_num):
            node_value = self.value["node"][
                node_id
            ]  # gdb.value representing kmem_cache->node[node_id] (struct kmem_cache_node *)
            kmem_cache_node = kcn.kmem_cache_node(
                self.sb, node_id, kmem_cache=self, value=node_value
            )
        ...
        for slab_index, full_slab_value in enumerate(full_slabs_values):
            full_slab = p.page(
                ...
                None,
                None,
                sb.SlabType.FULL_SLAB,
                index=slab_index + 1,
                count=slab_count,
                value=full_slab_value,
            )
    ...
    def print(self, verbose=0, use_cache=False, cmd=None):
        ...
        if cmd.args.object_only is not True:
            ...
            txt += "\n  {:11} = ".format("flags")
            flags_list = sb.Slub.get_flags_list(self.flags)
            if flags_list:
            ...
            txt += pu.color_value(
                "{:#d} ({:#x})".format(self.object_size, self.object_size)
            )
            txt += "\n"
            print(txt, end="")
        ...
        for kmem_cache_cpu in self.kmem_cache_cpu_list:
            ...
            if kmem_cache_cpu.main_slab is None:
                continue
            if (
                cmd.output_filtered is False
                or cmd.args.main_slab is True
                or cmd.args.partial_slab is True
            ) and (cmd.args.cpu is None or int(cmd.args.cpu) == kmem_cache_cpu.cpu_id):
                kmem_cache_cpu.print(indent=2, cmd=cmd)

        for kmem_cache_node in self.kmem_cache_node_list:
            if (
                cmd.cpu_filtered is False and cmd.output_filtered is False
            ) or cmd.args.node_slab is True:
                kmem_cache_node.print(indent=2, cmd=cmd)

        if (
            cmd.cpu_filtered is False and cmd.output_filtered is False
        ) or cmd.args.full_slab is True:
            # XXX - Ideally we would need to track the full slabs per kmem_cache_node
            # but we don't have that granularity yet
            if cmd.args.object_only is not True:
                txt = "  "
